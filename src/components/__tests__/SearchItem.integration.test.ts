...
import WdSearchItem from '../form/SearchItem.vue'
...
describe('WdSearchItem 搜索子项（label 帮助提示）', () => {
  it('不传 tip：等同普通表单项，渲染 label 与控件，无问号图标/帮助气泡', () => {
    const wrapper = mount(WdSearchItem, {
      props: { label: '姓名', prop: 'name' },
      slots: { default: h('input', { 'data-k': 'name' }) },
      global: { stubs }
    })

    expect(wrapper.classes()).toContain('wd-search-item')
    ...
    // 无帮助图标
    expect(wrapper.find('.wd-search-item__tip').exists()).toBe(false)
  ...
  it('传 tip：label 文字右侧自动出现问号图标，悬停解释内容等于 tip', () => {
    ...
    const tipEl = wrapper.find('.wd-search-item__tip')
  ...
  it('支持 #label 插槽覆盖默认 label 文字', () => {
    ...
    expect(wrapper.find('.el-form-item__label-stub .wd-search-item__title b').text()).toBe('自定义标签')
    wrapper.unmount()
  })
})
